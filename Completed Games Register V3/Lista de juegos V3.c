	...
	if(caracter >= '0' && caracter <= '9'){												/* El nombre del juego empieza con un numero */
		...
		while(temporal != NULL && existe){
			...
			if(caracter < '0' || caracter > '9') existe = false;
				...
				fprintf(archivo,"%s\"><i>%s</i></a>",temporal->juego.nombre,temporal->juego.nombre);	/* Ingresa ID del juego para la navegacion e ingresa el nombre del juego en indice*/
				temporal = temporal->sig;
			}
		}
		fprintf(archivo,".<br>");
...
bool generarHTML(nodo *lista, char filename[N]){
	...
	if((archivo = fopen(filename,"w")) != NULL){
		if((plantilla = fopen("playgames_data\\first_message_template.html","r")) != NULL){
			...
			while(temporal != NULL){
				fprintf(archivo,"<table class=\"estiloco\" id=\"%s\"><tr><th colspan=\"2\" class=\"title\">%s</th></tr>\n",temporal->juego.nombre,temporal->juego.nombre);					/* Titulo sin caracteres ilegales y abre tabla */
...
void parameters(nodo **lista, int *cantidad, bool *load_on_start, bool *save_on_close){ /* No se ha comprobado su funcionamiento y falta agregar casos (si/no abrio el archivo) */
	FILE *parameter;
